...
这个 skill 只要求“能为每段得到一张视觉图”，不把工作流绑定到某个供应商或具体工具名。**当前 agent 自检发现能生图并能落盘时，生图是主路径**；没有时再考虑 adapter；仍没有、不能落盘、或连续失败时，才用结构化 SVG scene 兜底。
...
这是跨 agent 的强制步骤,不能用 shell 是否检测到生图能力来代替。`check-deps.sh` 只能检测 CLI / 环境变量 adapter,检测不到对话工具、MCP、插件或 IDE 原生能力。

1. **枚举候选能力**:查看当前会话暴露的工具、skills、plugins、MCP、IDE 能力和可发现工具。名称不重要,语义上能"generate image / edit image / create bitmap / render asset"即可列为候选。不要只查 `image_gen`、`dalle`、`gpt-image` 这几个名字。
2. **判断能否落盘**:候选能力必须能直接保存、或能让 agent 把结果保存/复制到工作区。最终文件必须是 `deck/assets/img/sec-{N}.png`、`.jpg`、`.jpeg`、`.webp` 或同目录下 `assemble-deck.py` 能识别的图片。
3. **第 1 张即探针**:不要生成无关测试图浪费成本。用 `sec-1` 的正式 `image_prompt` 做探针,目标路径 `deck/assets/img/sec-1.png`。探针成功后再批量生成后续段落。
4. **验证探针结果**:确认文件存在、非空、可读取、接近 16:9、不是纯空白或聊天附件占位。如果能视觉检查,检查标题/标签错字和字幕安全区。
5. **记录决策**:在最终报告或 `deck/research/notes.md` 附近说明 `visual_backend`: `agent-native:<tool>` / `gpt-image-2` / `openai-api` / `gemini` / `svg-fallback`,并写明 fallback 原因。

只有下面情况允许跳过 agent-native 生图:

- 当前会话没有任何可调用的生图/修图/位图渲染能力。
- 生图工具只能在聊天里返回临时图片,agent 没有办法把它保存成项目文件。
- 用户明确要求不要生图、只要 SVG、或不允许使用某个付费/外部后端。
- 工具需要付费 API,但用户没有预先配置凭证或明确授权。
- 探针或后续生成连续失败,或文字/构图错误严重,一次定向修复后仍不可用。
...
执行时必须在最终报告里写清楚 `visual_backend`：`agent-native:<tool>` / `gpt-image-2` / `openai-api` / `gemini` / `svg-fallback`。如果是 `svg-fallback`,必须说明具体原因,例如"未发现可调用生图工具"、"工具无法落盘"、"用户禁用"、"adapter 未授权"或"探针失败"。
...
3. 如果后端不支持 reference image，把固定风格后缀追加到每段 prompt：`保持与第 1 张相同的 Excalidraw 手绘白板信息图风格：白底、黑色手绘马克笔线条、轻微抖动、柔和浅色填充、简单图标/箭头/卡片/流程线、中文大字清晰可读、留足空白、不要 3D/照片感/复杂背景/品牌 logo/水印`。
...
风格：<Excalidraw 手绘白板信息图 / editorial cinematic / clean keynote>
线条与颜色：<黑色手绘马克笔线条，轻微抖动；浅蓝、浅黄、浅绿、浅紫等柔和填充>
限制：no watermark, no random text, no tiny unreadable paragraphs, no cropped title, no 3D, no photorealism, no complex background, lower third clean for subtitles
```

LLM 可以自由发挥画面隐喻，但不要省略文字清单、版式、主体、空间关系、线条和颜色规则。
...
## 默认风格：excalidraw-whiteboard

用于行业研究、技术解释、中文课程讲义和知识付费文档的默认风格，目标是“像老师在白板上画出来的结构图”，不是海报、照片或 3D 画面。
