...
| `edge-tts` | 硬依赖 | `which edge-tts` | `uv tool install edge-tts`(或 `pip install --user edge-tts`) |
...
- edge-tts 网络失败 → 重试 3 次,仍失败则建议换 voice 或检查代理;**注意**:TTS 失败时 HTML 仍可播,母版会用每段的 `data-narration` 文本作为静态字幕兜底(无声但有文字)
