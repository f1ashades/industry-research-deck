...
description: |
  ...
  整合**联网搜索能力**(优先 Tavily 深度检索;agent 自带的 web_search / web_fetch 是合格的下位替代;也支持用户直接喂资料)、edge-tts 配音、当前 agent 生图能力(优先 imagegen/image-2;否则 adapter;最后结构化 SVG scene 兜底)、HTML 母版组装。
...
| **视觉方式** | 当前 agent 能生图就优先 imagegen/image-2；不能生图或失败再 SVG scene | 用户明确指定“不要生图/只要 SVG”时改 |
...
- 视觉（默认读）：先读 [references/image-generators.md](references/image-generators.md) 写 imagegen/image-2 级别的主视觉 prompt；再写 `visual` 作为 SVG fallback scaffold。只有无生图能力或生图失败时才读 [references/ppt-scene-system.md](references/ppt-scene-system.md) 并执行 SVG scene。
...
**视觉分镜提示**：当前环境有 `image_gen` / `$imagegen` 能力时，必须优先生成 raster 主视觉。`visual` 不是默认产物，而是为了生图失败、无生图能力、或精确文字持续出错时的兜底。连续段落要轮换构图（生态图 / 对比图 / 流程图 / 仪表盘 / 结论页），避免每张都是同一种中央图。
...
先走 [references/image-generators.md](references/image-generators.md)。当前 agent 有内置 `image_gen` / `$imagegen` 能力时，直接使用它为每段生成 `deck/assets/img/sec-{N}.png`；不要因为 SVG 更可控就跳过生图。只有当前环境确实不能生图、用户明确要求不用生图、或生图连续失败时，才执行 SVG fallback。

- 当前 agent 有内置 image generation tool 时，按 `$imagegen` skill 的保存策略执行：先生成，检查输出，把最终选中的图片复制/移动到 `deck/assets/img/sec-{N}.png`。项目引用的图片不能只留在 `$CODEX_HOME/generated_images/...`。
