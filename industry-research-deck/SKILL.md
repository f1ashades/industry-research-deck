...
0. 依赖自检   →  1. 澄清需求    →  2. 信息检索   →  3. 拆段写脚本 + 视觉分镜
                                                                       ↓
6. 本地预览  ←  5. HTML 组装   ←  4a/4b. 并行全屏视觉 + 配音
```
  ...
  "sections": [
    {
      ...
      "visual_prompt": "1920x1080 full-bleed 讲解视频 scene，<具体主体和动作>，<空间关系>，<镜头/构图>，<光线和色彩>，主体占画面 65%-85%，下方 30% 字幕安全区干净，no title card, no long text",
...
**视觉分镜提示**：写 `visual_prompt` 前先读 [references/visual-workflow.md](references/visual-workflow.md) 的分镜规则。每段画面必须是 full-bleed scene，不是小插图、标题卡或 PPT 套娃。连续段落要轮换构图（左右对比 / 管线 / 工作台 / 仪表盘 / 风险闸门 / 人物空间隐喻），避免每张都是中央小节点图。
...
详见 [references/image-generators.md](references/image-generators.md) 和 [references/visual-workflow.md](references/visual-workflow.md)。生图是“能力”而不是必须绑定某个供应商：
...
**硬性视觉规则**：
- 每段画面必须是“场景 / 机制 / 关系”的 full-bleed 可视化，覆盖 16:9 画布；不要生成标题卡、PPT 内嵌页、纯文字图、只有大字的封面图或中间小卡片。
- prompt 要明确“主体占画面 65%-85%，下方 28%-32% 字幕安全区保持干净”。HTML 模板会把图片作为整页背景层，不再把它限制成插图。
- prompt 必须包含主体、动作、环境/空间关系、构图、镜头/视角、光线、色彩和限制项；详见 [references/image-generators.md](references/image-generators.md)。
- 默认不要让图像模型渲染大段文字。若必须有字，只允许 1-3 个短标签，并在 HTML 标题/字幕里承载主要信息。
- 占位图只是低保真 storyboard，不是最终视觉；但 fallback 也必须铺满画布并可视化机制。能生图时必须优先生图；无法生图时才用 `generate-placeholders.py`。

**风格一致性**：第一张作为 reference，后续段在 prompt 末尾追加「保持与第一张相同的视觉风格、色彩、镜头语言和材质」。如果不能传 reference，复制固定风格后缀。
...
<section data-id="sec-1" data-duration="20"
  ...
  <img src="assets/img/sec-1.png" alt="..."> <!-- full-bleed 背景视觉 -->
  <h2>...标题...</h2>
</section>
```
...
预览后必须做一次浏览器/HTTP 验证：确认 `deck.html` 返回 200、图片全部加载且覆盖整个 section、音频和字幕数量等于段数、桌面和移动端没有标题/字幕/来源互相遮挡。图片现在是 full-bleed 背景层，不把“图片和字幕重叠”当作错误；真正要查的是字幕是否可读、是否被裁切。若后台服务在 shell 退出后被带掉，在 macOS 可用 `screen -dmS deck-preview bash -lc 'cd deck && python3 -m http.server 8765'`。
