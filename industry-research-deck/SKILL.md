  ...
  "sections": [
    {
      ...
      "visual_prompt": "editorial-cinematic 风格，<具体主体和动作>，<空间关系>，<镜头/构图>，<光线和色彩>，16:9，字幕安全区留空",
...
**硬性视觉规则**：
- 每段画面必须是“场景 / 机制 / 关系”的可视化，不要生成标题卡、PPT 内嵌页、纯文字图、只有大字的封面图。
...
- 占位图只是低保真 storyboard，不是最终视觉。能生图时必须优先生图；无法生图时才用 `generate-placeholders.py`。
...
预览后必须做一次浏览器/HTTP 验证：确认 `deck.html` 返回 200、图片全部加载、音频和字幕数量等于段数、桌面和移动端没有标题/字幕重叠。若后台服务在 shell 退出后被带掉，在 macOS 可用 `screen -dmS deck-preview bash -lc 'cd deck && python3 -m http.server 8765'`。
